...
parser.add_argument('--dropout', type=float, default=0.12, help='Dropout rate (1 - keep probability).')
...
args.sceneHG=False
adj, dis, features, labels, idx_train, idx_val, idx_test, SPidx,idx,labelweight= KNN()
...
args.AblationGroup = "d"
if args.AblationGroup == "a":
    adj1 = dis
    adj2 = dis
elif args.AblationGroup == "b":
    # args.epochs=5000
    adj1 = dis
    adj2 = adj
elif args.AblationGroup == "c":
    adj1 = (adj+dis)/2
    adj2 = (adj+dis)/2
elif args.AblationGroup == "d":
    adj1 = adj
    adj2 = dis
elif args.AblationGroup == "e":
    adj1 = adj
    adj2 = adj
...
def train():
    model.train()
    optimizer.zero_grad()
    output = model(features, adj1, adj2)
    ...
    loss_train.backward()
    optimizer.step()
    return loss_train.item(),acc_train.item(),adj
...
if args.Visualisation:
    ...
    print("Visualisation...")
    if args.sceneHG:
        paint(xyz,outlabel,txtname='./Points/HT/SSGCN_{}{}.txt'.format(args.percenttrain,args.AblationGroup))
    else:
        paint(xyz,outlabel,txtname='./Points/UH/SSGCN_{}{}.txt'.format(args.percenttrain,args.AblationGroup))
    print("Visualisation Finished!")
    NCM=normalize_adj(CM)
    
    if args.sceneHG:
        np.savetxt('./confuseHT/SSGCN_{}{}.txt'.format(args.percenttrain,args.AblationGroup),NCM,delimiter=" ",fmt="%.2f")
        ...
        np.savetxt('./confuseUH/SSGCN_{}{}.txt'.format(args.percenttrain,args.AblationGroup),NCM,delimiter=" ",fmt="%.2f")
        ax= sns.heatmap(pd.DataFrame(NCM), annot=True,square=True, cmap="YlGnBu",
